...
const float MOUSE_RADIUS = 40.0;

const float SPEED_FF = (1.0 / 6.0); // tune to make average speed 
const float ROTATION_BIAS = 0.002; // to make robot go forward
...
//** SENSORS **/
const int LEFT_CALIBRATION = 175; // test when robot centered and no front wall
const int RIGHT_CALIBRATION = 175; // test when robot centered and no front wall
const int FRONT_CALIBRATION_LEFT = 195; // test when robot with back walls
...
const float FRONT_SCALE = (float)NOMINAL_VALUE / ((FRONT_CALIBRATION_LEFT + FRONT_CALIBRATION_RIGHT) * 0.5);
...
const int FRONT_THRESHOLD = 125;

// value to decide if we in cell center
const int FRONT_REFERENCE = 400;
